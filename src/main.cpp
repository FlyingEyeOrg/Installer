...
void AddControl(HWND hWnd, LPCWSTR className, LPCWSTR text, DWORD style, int x,
...
void AddControl(HWND hWnd, LPCWSTR className, LPCWSTR text, DWORD style, int x,
                int y, int width, int height, int id) {
    HWND hControl =
        CreateWindowW(className, text, WS_CHILD | WS_VISIBLE | style, x, y,
                      width, height, hWnd, (HMENU)(INT_PTR)id, hInst, nullptr);
    if (hControl) {
        if (id != 0) {
...
void CreateTabPages(HWND hTab) {
    ...
    // 页面1
    HWND hPage1 = CreateWindowW(L"STATIC", L"", WS_CHILD | WS_VISIBLE,
    ...
    HWND hPage1Text =
        CreateWindowW(L"STATIC",
                      L"页面1内容区域\n\n这是第一个标签页\n可以放置相关控件和内"
                      L"容\n\n当前页面: 基本信息",
                      WS_CHILD | WS_VISIBLE | SS_CENTER, 10, 30, pageWidth - 20,
                      120, hPage1, nullptr, hInst, nullptr);
    SetFontForControl(hPage1Text);

    // 页面2
    HWND hPage2 = CreateWindowW(L"STATIC", L"", WS_CHILD, rcTab.left, rcTab.top,
                                pageWidth, pageHeight, hTab, (HMENU)ID_PAGE2,
                                hInst, nullptr);
    ...
    HWND hPage2Text =
        CreateWindowW(L"STATIC",
                      ...
                      L"当前页面: 配置选项",
                      WS_CHILD | WS_VISIBLE | SS_CENTER, 10, 30, pageWidth - 20,
                      120, hPage2, nullptr, hInst, nullptr);
    SetFontForControl(hPage2Text);

    // 页面3
    HWND hPage3 = CreateWindowW(L"STATIC", L"", WS_CHILD, rcTab.left, rcTab.top,
                                pageWidth, pageHeight, hTab, (HMENU)ID_PAGE3,
                                hInst, nullptr);
    ...
    HWND hPage3Text =
        CreateWindowW(L"STATIC",
                      ...
                      L"\n当前页面: 关于",
                      WS_CHILD | WS_VISIBLE | SS_CENTER, 10, 30, pageWidth - 20,
                      120, hPage3, nullptr, hInst, nullptr);
...
void CreateAllControls(HWND hWnd) {
    ...
    // 1. 标题
    AddControl(hWnd, L"STATIC", L"===基本控件===", SS_LEFT | SS_CENTERIMAGE, x,
               y, COLUMN_WIDTH - 40, ROW_HEIGHT, 0);
    y += ROW_HEIGHT + SPACING;

    // 2. 按钮
    AddControl(hWnd, L"BUTTON", L"普通按钮", BS_PUSHBUTTON, x, y, 120,
               ROW_HEIGHT, ID_BUTTON_1);
    AddControl(hWnd, L"BUTTON", L"默认按钮", BS_DEFPUSHBUTTON, x + 130, y, 120,
               ROW_HEIGHT, ID_BUTTON_2);
    y += ROW_HEIGHT + SPACING;

    // 3. 单行文本框
    AddControl(hWnd, L"STATIC", L"单行文本:", SS_RIGHT | SS_CENTERIMAGE, x, y,
               80, ROW_HEIGHT, 0);
    AddControl(hWnd, L"EDIT", L"可编辑文本", WS_BORDER | ES_AUTOHSCROLL, x + 90,
               y, 200, ROW_HEIGHT, ID_EDITBOX);
    y += ROW_HEIGHT + SPACING;

    // 4. 多行文本框
    AddControl(hWnd, L"STATIC", L"多行文本:", SS_RIGHT | SS_CENTERIMAGE, x, y,
               80, ROW_HEIGHT, 0);
    AddControl(hWnd, L"EDIT", L"多行文本输入区域",
               WS_BORDER | ES_MULTILINE | ES_AUTOVSCROLL | WS_VSCROLL, x + 90,
               y, 200, 60, 0);
    y += 70;

    // 5. 复选框和单选按钮
    AddControl(hWnd, L"BUTTON", L"复选框选项", BS_AUTOCHECKBOX, x, y, 120,
               ROW_HEIGHT, ID_CHECKBOX);
    y += ROW_HEIGHT + SPACING;

    AddControl(hWnd, L"BUTTON", L"单选按钮1", BS_AUTORADIOBUTTON | WS_GROUP, x,
               y, 100, ROW_HEIGHT, ID_RADIO_1);
    AddControl(hWnd, L"BUTTON", L"单选按钮2", BS_AUTORADIOBUTTON, x + 110, y,
               100, ROW_HEIGHT, ID_RADIO_2);
    AddControl(hWnd, L"BUTTON", L"单选按钮3", BS_AUTORADIOBUTTON, x + 220, y,
               100, ROW_HEIGHT, ID_RADIO_3);
    y += ROW_HEIGHT + SPACING;

    // 6. 分组框
    int groupBoxWidth = 320;
    int groupBoxHeight = 100;
    AddControl(hWnd, L"BUTTON", L"设置分组", BS_GROUPBOX, x, y, groupBoxWidth,
               groupBoxHeight, ID_GROUPBOX);
    ...
    AddControl(hWnd, L"EDIT", L"组内编辑框", WS_BORDER, groupInnerX + 90,
               groupInnerY, 150, ROW_HEIGHT, 0);
    AddControl(hWnd, L"BUTTON", L"组内按钮", BS_PUSHBUTTON, groupInnerX + 250,
               groupInnerY, 60, ROW_HEIGHT, 0);
    y += groupBoxHeight + SPACING;

    // 7. 列表框
    AddControl(hWnd, L"STATIC", L"列表框:", SS_LEFT | SS_CENTERIMAGE, x, y, 80,
               ROW_HEIGHT, 0);
    AddControl(hWnd, L"LISTBOX", L"", WS_BORDER | LBS_NOTIFY, x, y + 25, 320,
               100, ID_LISTBOX);
    y += 130;

    // 8. 组合框
    AddControl(hWnd, L"STATIC", L"下拉框:", SS_LEFT | SS_CENTERIMAGE, x, y, 80,
               ROW_HEIGHT, 0);
    AddControl(hWnd, L"COMBOBOX", L"", WS_BORDER | CBS_DROPDOWNLIST, x + 90, y,
               200, 200, ID_COMBOBOX);
    y += ROW_HEIGHT + SPACING;
    ...
    // 1. 标题
    AddControl(hWnd, L"STATIC", L"===高级控件===", SS_LEFT | SS_CENTERIMAGE, x,
               y, COLUMN_WIDTH - 40, ROW_HEIGHT, 0);
    y += ROW_HEIGHT + SPACING;

    // 2. 进度条
    AddControl(hWnd, L"STATIC", L"进度条:", SS_RIGHT | SS_CENTERIMAGE, x, y, 80,
               ROW_HEIGHT, 0);
    AddControl(hWnd, PROGRESS_CLASSW, L"", 0, x + 90, y, 200, 20,
               ID_PROGRESSBAR);
    y += ROW_HEIGHT + SPACING;

    // 3. 滑动条
    AddControl(hWnd, L"STATIC", L"滑动条:", SS_RIGHT | SS_CENTERIMAGE, x, y, 80,
               ROW_HEIGHT, 0);
    AddControl(hWnd, TRACKBAR_CLASSW, L"", TBS_AUTOTICKS | TBS_HORZ, x + 90, y,
               200, 30, ID_TRACKBAR);
    y += 40;

    // 4. 数值调节
    AddControl(hWnd, L"STATIC", L"数值调节:", SS_RIGHT | SS_CENTERIMAGE, x, y,
               80, ROW_HEIGHT, 0);

    // 创建数值调节控件
    HWND hEditSpin = CreateWindowW(
        L"EDIT", L"57", WS_BORDER | ES_NUMBER | WS_VISIBLE | WS_CHILD, x + 90,
        y, 100, ROW_HEIGHT, hWnd, (HMENU)ID_EDIT_SPIN, hInst, nullptr);
    SetFontForControl(hEditSpin);

    HWND hUpDown =
        CreateWindowW(UPDOWN_CLASSW, L"",
    ...
    // 5. 日期时间选择器
    AddControl(hWnd, L"STATIC", L"日期时间:", SS_RIGHT | SS_CENTERIMAGE, x, y,
               80, ROW_HEIGHT, 0);
    AddControl(hWnd, DATETIMEPICK_CLASSW, L"", DTS_SHORTDATEFORMAT, x + 90, y,
               200, ROW_HEIGHT, ID_DATETIME_PICKER);
    y += ROW_HEIGHT + SPACING;

    // 6. 月历控件
    AddControl(hWnd, L"STATIC", L"月历:", SS_LEFT | SS_CENTERIMAGE, x, y, 80,
               ROW_HEIGHT, 0);
    AddControl(hWnd, MONTHCAL_CLASSW, L"", MCS_DAYSTATE, x, y + 25, 300, 160,
               ID_MONTH_CALENDAR);
    y += 190;

    // 7. IP地址控件
    AddControl(hWnd, L"STATIC", L"IP地址:", SS_RIGHT | SS_CENTERIMAGE, x, y, 80,
               ROW_HEIGHT, 0);
    AddControl(hWnd, WC_IPADDRESSW, L"", 0, x + 90, y, 200, ROW_HEIGHT,
               ID_IP_ADDRESS);
    y += ROW_HEIGHT + SPACING;

    // 8. 热键控件
    AddControl(hWnd, L"STATIC", L"热键:", SS_RIGHT | SS_CENTERIMAGE, x, y, 80,
               ROW_HEIGHT, 0);
    AddControl(hWnd, HOTKEY_CLASSW, L"", 0, x + 90, y, 200, ROW_HEIGHT,
               ID_HOTKEY);
    y += ROW_HEIGHT + SPACING;
    ...
    // 1. 标题
    AddControl(hWnd, L"STATIC", L"===容器控件===", SS_LEFT | SS_CENTERIMAGE, x,
               y, COLUMN_WIDTH - 40, ROW_HEIGHT, 0);
    y += ROW_HEIGHT + SPACING;

    // 2. 标签页控件
    AddControl(hWnd, WC_TABCONTROLW, L"", TCS_FIXEDWIDTH, x, y, 320, 200,
               ID_TAB_CONTROL);
    ...
    // 3. 树形视图
    AddControl(hWnd, L"STATIC", L"树形视图:", SS_LEFT | SS_CENTERIMAGE, x, y,
               80, ROW_HEIGHT, 0);
    AddControl(hWnd, WC_TREEVIEWW, L"",
               WS_BORDER | TVS_HASBUTTONS | TVS_LINESATROOT, x, y + 25, 320,
               150, ID_TREEVIEW);
    y += 180;

    // 4. 列表视图
    AddControl(hWnd, L"STATIC", L"列表视图:", SS_LEFT | SS_CENTERIMAGE, x, y,
               80, ROW_HEIGHT, 0);
    AddControl(hWnd, WC_LISTVIEWW, L"", WS_BORDER | LVS_REPORT, x, y + 25, 320,
               150, ID_LISTVIEW);
    y += 180;

    // === 状态栏 ===
    HWND hStatusBar = CreateWindowExW(
        0, STATUSCLASSNAMEW, L"", WS_CHILD | WS_VISIBLE | SBARS_SIZEGRIP, 0, 0,
        0, 0, hWnd, (HMENU)ID_STATUSBAR, hInst, nullptr);
...
void InitializeControls(HWND hWnd) {
    ...
    if (hIPAddress) {
        SendMessage(hIPAddress, IPM_SETADDRESS, 0,
                    MAKEIPADDRESS(192, 168, 1, 1));
    }
...
void OnButtonClick(HWND hWnd, int buttonId) {
    switch (buttonId) {
        ...
        case ID_BUTTON_2: {
            BOOL checked =
                SendMessage(g_controls[ID_CHECKBOX], BM_GETCHECK, 0, 0);
            std::wstring message =
                checked ? L"复选框已选中！" : L"复选框未选中！";
...
void OnSelectionChange(HWND hWnd, int controlId) {
    switch (controlId) {
        case ID_LISTBOX: {
            ...
            if (index != LB_ERR) {
                wchar_t text[100];
                SendMessage(g_controls[ID_LISTBOX], LB_GETTEXT, index,
                            (LPARAM)text);
        ...
        case ID_COMBOBOX: {
            int index =
                SendMessage(g_controls[ID_COMBOBOX], CB_GETCURSEL, 0, 0);
            if (index != CB_ERR) {
                wchar_t text[100];
                SendMessage(g_controls[ID_COMBOBOX], CB_GETLBTEXT, index,
                            (LPARAM)text);

                std::wstring message = L"选择了: ";
                message += text;
                SendMessage(g_controls[ID_LISTBOX], LB_ADDSTRING, 0,
    ...
    switch (message) {
        ...
        case WM_COMMAND: {
            ...
            // 处理列表框双击
                ...
                int index =
                    SendMessage(g_controls[ID_LISTBOX], LB_GETCURSEL, 0, 0);
                if (index != LB_ERR) {
                    wchar_t text[100];
                    SendMessage(g_controls[ID_LISTBOX], LB_GETTEXT, index,
                                (LPARAM)text);
        ...
        case WM_HSCROLL: {
            if (LOWORD(wParam) == SB_THUMBTRACK &&
                g_controls.find(ID_TRACKBAR) != g_controls.end()) {
